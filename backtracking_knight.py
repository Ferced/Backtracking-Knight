...
class KnightsTour():
    ...
    def start_pg(self):
        board = ([[self.size[0]*y + x for x in range(1,self.size[1]+1)] for y in range(self.size[1])])
        y_counter = 0
        for row in board:
            x_counter=0
            for column in row:
    ...
    def draw_board(self,current_position,history,posible_moves):
        for y in range(0, self.board_height, self.tile_size):
            ...
            next(self.colors)
        self.draw_shit(self.background)

    def start(self):
        board = ([[8*y + 1*x for x in range(1,9)] for y in range(8)])
        current_position = [7,4]
        cicle = 1
        self.next_position(board,current_position,cicle)
    ...
    def jump_validation(self,board,move):
        for index in move:
            if int(index) < 0 or int(index) > self.size[0]-1:
                return False
        if "(" in str(board[move[0]][move[1]]):
            return False
        #print ("Movimiento validado:",move)
        return True
    ...
    def posible_moves_pos(self,board,current_position):
        posible_moves_list = []
    ...
    def next_position_draw(self,board,current_position,cicle,history):
        ...
        #self.print_board(board)
    #SE MUEVE A LA PROXIMA POSICION
    def next_position(self,board,current_position,cicle):
        if self.board_finished(board):
            print ("----------------TABLERO TERMINADO-----------------")
            self.print_board(board)
            print ("----------------TABLERO TERMINADO-----------------")
        ...
        counter=0
        while len(posible_moves)>0:
            result =self.next_position([list(row) for row in board],posible_moves[counter],cicle+1)
            if result == False:
                posible_moves.pop(counter)
                counter-=1
            counter+=1
